...
export const CertificateEditor = () => {
  ...
  const handleDelete = async (id, imageUrl) => {
    ...
    try {
      await portfolioService.deleteItem("certificates", id);
      if (imageUrl) {
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      {certificates.length === 0 && <p className="text-gray-500 italic">{t("common.noData")}</p>}

      <Modal isOpen={isModalOpen} onClose={() => !saving && setIsModalOpen(false)} title={editingItem ? (lang === "th" ? "แก้ไขใบประกาศ" : "Edit Certificate") : (lang === "th" ? "เพิ่มใบประกาศ" : "Add Certificate")}>
        <form onSubmit={handleSubmit} className="space-y-4 py-2">
          ...
          <div className="flex flex-col space-y-1.5">
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{lang === "th" ? "รูปภาพใบประกาศ" : "Certificate Image"}</label>
            <input 
              type="file" 
              accept="image/*" 
